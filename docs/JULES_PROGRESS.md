...
## Recently completed
- **Clinical Safety:** Corrected the interpersonal convergence rule (`N3_convergencia_interpersonal_despedida`) in `backend/app/services/risk_engine.py` to trigger a Level 3 professional review rather than an automatic Level 4 emergency, adhering to clinical safety guidelines and avoiding unvalidated emergency escalation for indirect signals.
