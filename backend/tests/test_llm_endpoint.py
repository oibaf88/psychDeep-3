...
class ProviderSelectionTests(unittest.TestCase):
    ...
    def test_anthropic_environment_profile_and_key_never_serialises(self):
        ...
        self.assertEqual(config.chat_model, "claude-sonnet-5")
    ...
    def test_validation_accepts_only_the_two_intended_providers(self):
        with patch.object(llm_config, "backend_runtime", return_value="local"):
            claude = llm_config.validate(
                provider="anthropic", base_url="http://ignored.example/v1", chat_model="claude-sonnet-5",
                analysis_model="claude-sonnet-5", max_tokens=8192, timeout_seconds=300,
            )
